          ...
          <>
            <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
              <Figure
                ...
                tone="text-allow"
                hint="every one executed against the real gate"
              />
              <Figure
                value={String(adversarial.techniques)}
                label="Distinct attack techniques"
                tone="text-ink"
                hint="each run against many items, tiers and amounts"
              />
              <Figure
              ...
              <Figure
                ...
                hint={`${benign.escalated_to_human} escalated to the human by design`}
              />
              <Figure
                value={
                ...
                tone="text-brand"
                hint={defence ? `across ${defence.total} synthetic disputes` : undefined}
              />
            </div>
            <p className="mt-5 text-[12px] text-faint">
              Regenerated by <code className="font-mono">python -m app.cli reports</code>, which
              runs every case end to end. A technique is one attack idea; a case is that idea
              executed against one item, issuing tier and amount, so the two are counted separately
              and neither stands in for the other. The escalations in the benign corpus are counted
              apart from false positives: asking the principal about a constraint the kernel cannot
              decide is the designed behaviour, not an error.
            </p>
          </>
        ) : (
...
const LIMITS = [
  ...
  {
    title: "Multi-hop delegation is not implemented",
    body: "An agent here presents credentials issued directly to it. A chain where one agent delegates to another, each narrowing the authority as it goes, is single-hop only: the SD-JWT work is done, the delegation semantics are not. The conformance matrix in the README says so rather than implying otherwise.",
  },
  {
