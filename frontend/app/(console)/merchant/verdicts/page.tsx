...
export const metadata: Metadata = { title: "Verdict log" };
...
export default async function VerdictsPage({
  ...
  const href = (decision?: string) => {
    ...
    return suffix ? `/merchant/verdicts?${suffix}` : "/merchant/verdicts";
  };
  ...
  return (
    <>
      <PageHeader
        title="Verdict log"
        description="Every policy decision, with the reason code it carries and the evidence it was made on. Refusals are shown as prominently as approvals."
      />
        ...
        <Table head={["When", "Agent", "Action", "Decision", "Reason code", "Agent should", "Amount", "Evidence"]}>
          {verdicts.map((verdict) => (
            <Row key={verdict.id}>
              ...
              <Cell mono>
                <Link
                  href={`/merchant/verdicts?agent_id=${encodeURIComponent(verdict.agent_id)}`}
                ...
                <details>
                  ...
                  <div className="mt-2 max-w-xl space-y-2">
                    {verdict.correlation_id && (
                      <Link
                        href={`/merchant/evidence/${verdict.correlation_id}`}
