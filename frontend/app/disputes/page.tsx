...
import { BackendDown, Card, Cell, Empty, Pill, Row, Stat, Table } from "@/components/ui";
  ...
  return (
    <>
      ...
      <Card
        ...
        description="Each representment states what authority was presented, what the human constrained, how the cart satisfied it, and what the buyer acknowledged. Where the evidence is weak, it says to refund rather than contest."
      >
        {disputes.length === 0 ? (
          <Empty>
          ...
          >
            {disputes.map((dispute) => (
              <Row key={dispute.id}>
                <Cell>
                  <Link href={`/disputes/${dispute.id}`} className="text-accent hover:underline">
                    {dispute.claim}
                  </Link>
                </Cell>
                <Cell>
                  <Link
                    href={`/evidence/${dispute.correlation_id}`}
